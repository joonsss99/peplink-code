...
if [ "$FW_CONFIG_KDUMP" = "y" ]; then
	...
	$PACKAGESDIR/$PACKAGE/get_struct_log.awk $FETCHEDDIR/kernel/printk.c >> $dmesg_inc
	...
	echo "static uint64_t log_sz = sizeof(struct log);" >> $dmesg_inc
	echo "static uint64_t log_offset_ts_nsec = offsetof(struct log, ts_nsec);" >> $dmesg_inc
	echo "static uint16_t log_offset_len = offsetof(struct log, len);" >> $dmesg_inc
	echo "static uint16_t log_offset_text_len = offsetof(struct log, text_len);" >> $dmesg_inc
fi
